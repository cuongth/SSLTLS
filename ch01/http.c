...
/**
 * Accept a well-formed URL and return pointers to the host part
 * and the path part. Note that this function modifies the uri itself as well.
 * It returns 0 on success, 1 if URL is found to be malformed in any way.
 */
int parse_url(const char *uri, char **host, char **path);

/**
 * Accept a proxy URL and return poniters to the proxy host, proxy port,
 * proxy user, proxy password and return proxy port number. Note that
 * this function modifies the uri itself as well. It returns 0 on success,
 * error code if there's failure on proxy URL.
 */
 ...
 * success, error code on failure with error set appropriately.
 * The caller must then retrieve the response.
 */
int http_get( int connection, const char *path, const char *host,
    const char *proxy_host, char *proxy_user, char *proxy_password);

/**
 * Receive all data available on a connection and
 * dump it to stdout
 */
...
int main( int argc, char *argv[]) {
  ...
  // Step 1: open a socket connection on http port with
...
int http_get( int connection, const char *path, const char *host,
  ...
  if ( proxy_user ) {
    // TODO: support only BASIC method
...
int parse_proxy_param( char *proxy_spec,
  ...
  // If user add a '/' on the end, just ignore it.
  ...
  colon_sep = strchr( proxy_spec, ':' );
  if ( colon_sep ) { // non-standard proxy port
    *colon_sep = '\0';
