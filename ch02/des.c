 ...
 * respectly, but src can be shorter.
 * NOTE: this assumes that the permutation tables are defiend as one-based
 ...
 * complex by the fact that the key is split into two 28-bit halves,
 * each of wich has to be rotated independently
 */
static void rol( unsigned char *target ) {
  ...
  target[3] = ( ( (target[3] << 1) | ((target[4] & 0x80) >> 7) )
      & ~0x10 ) | carry_left;
  target[4] = ( target[5] << 1 ) | ( (target[6] & 0x80) >> 7 );
  target[5] = ( target[6] << 1 ) | carry_right;
}

/**
 * Perform the right rotation operation on the key.
 */
...
static void des_block_operate( const unsigned char plaintext[DES_BLOCK_SIZE],
  ...
  for ( round = 0; round < 16; round++ ) {
    ...
    // "Expansion" on left half of ip_block
    ...
    if ( operation == OP_ENCRYPT ) {
      ...
      if ( !(round <= 1 || round == 8 || round == 15) ) {
        // Rotate left twice except in rounds 1, 2, 9, 16
        rol( pc1key );
      }
    }
    ...
    if ( operation == OP_DECRYPT ) {
      ...
      if ( !(round >= 14 || round == 7 || round == 0) ) {
        // Rotate right twice except in rounds 16, 15, 8, 1
	ror( pc1key );
      }
    }
    ...
    // Recombination. XOR the pbox with left half then switch sides.
...
void des_decrypt( const unsigned char *input,
    int input_len, unsigned char *output,
    const unsigned char *key ) {
  des_operate( input, input_len, output, key, OP_DECRYPT );
}
...
/*
 ./des -e password abcdefgh
 929193989a999b94
 ./des -d password 0x929193989a999b94
6162636465666768
